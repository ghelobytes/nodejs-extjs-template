/* This Class is purely based on ArcGISCache. Sensible defaults were 
 * provided to work with Philippine Geoportal's Map Service
 ...
 */    
OpenLayers.Layer.NAMRIA = OpenLayers.Class(OpenLayers.Layer.XYZ, {  

	// forces some default settings on map so that the layer
	// should work without weird behaviour
	forceMapSettings: true,
    ...
    */ 
    initialize: function(name, url, options) { 
		...
		// start NAMRIA ArcGISCache defaults
		options.tileOrigin = new OpenLayers.LonLat(-20037508.342787,20037508.342787);
		...
		// end NAMRIA ArcGISCache defaults
		
        OpenLayers.Layer.XYZ.prototype.initialize.apply(this, arguments);

		// somehow the maps won't work properly if restrictedExtent is not set on the map object
		// therefore, force setting it whenever possible
		if(this.forceMapSettings){
			this.events.register('added', this, function(e){
				e.map.restrictedExtent = new OpenLayers.Bounds(11516520.903064,482870.29798867,15821300.345956,2448728.3963715);
				e.projection = 'EPSG:900913';
			});
		}
    ...
    */ 
    clone: function (obj) { 
        if (obj == null) { 
            obj = new OpenLayers.Layer.NAMRIA(this.name, this.url, this.options);
        }
    ...
    */
    ...
    CLASS_NAME: 'OpenLayers.Layer.NAMRIA' 
});